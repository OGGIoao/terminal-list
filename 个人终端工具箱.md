...
| 4 | `pycheat` | L5 Python CLI | **说人话也能命中**的离线速查 + **开场屏**（置顶「你可能关心的指令」+ 最近查看 + ↻该复习的）+ **自学习别名**（越用越懂你）+ 卡面分区配色：用途→用法→示例→通俗别名→踩坑→平台对照(macOS/Linux↔Windows) | 第三卷·Shell / 第六卷·git / 路线图阶段2 |
...
#   pycheat --llm <说法>   （预留）接 ollama 后启用本地语义匹配
