...
> 最后更新：2026-08-22（初建：工具 #1 cheat、工具 #2 hi；cheat 增强：按意图搜索 / 示例复制 / 敲错自动提示；**新增 git 实战 8 张命令卡**）
...
- **fzf**：已装（`/opt/homebrew/bin/fzf` v0.74.2）—— `cheat` 交互选择靠它（**需要真实终端**；非交互脚本环境里不弹窗）
...
| # | 工具 | 层级 | 一句话 | 关联 OS 课卷 |
|---|------|------|--------|--------------|
| 1 | `cheat`（别名 `c`） | L2 函数 | 我的命令速查，**按意图搜/示例直复制/敲错自提示** | 第二卷·文件系统 / 第三卷·Shell / 第四卷·环境变量分裂 |
...
| 4 | `pycheat` | L5 Python CLI | **说人话也能命中**的离线速查 + **开场屏**（置顶「你可能关心的指令」+ 最近查看），卡面分区配色：用途→用法→示例→通俗别名→踩坑→平台对照(macOS/Linux↔Windows) | 第三卷·Shell / 第六卷·git / 路线图阶段2 |
...
- **用途**：终端内即查即用的个人命令速查。核心设计是**不用背命令名**——记不住名字就按「想做的事」搜。
- **代码**（`~/.cheat.zsh`，已被 `~/.zshrc` source；增强版）：
...
- **关键技巧**：
  - `cheat` 有参数时：先精确匹配命令名，匹配不到就**自动当关键词搜**（命令名含词 或 卡片内容含词，含中文）。所以 `cheat 内存`、`cheat 磁盘`、`cheat sys` 都能命中。
  - `-c` 把卡片里的「示例」命令复制到剪贴板（macOS 用 `pbcopy`），你 `⌘V` 直接粘贴运行，**不用手敲**。
  - `command_not_found_handler`：zsh 在命令找不到时自动调用，主动提示 cheat 里相关的卡——**敲错命令也会被温柔提醒**。
  - **交互浏览模式**（2026-08-22 新增）：`cheat` 或 `cheat <关键词>` 进入浏览，底部常驻菜单——`n/→` 下一张、`p/←` 上一张、`b` 回退、`f` 前进（带历史栈）、`c` 复制示例、`/` 重新搜索、`q` 退出。不再「选完即退」。
  - 非交互 shell（脚本/沙箱/`$(...)`）里自动退化为「只展示当前卡」，不会卡住。
  - 底层用 `awk` 按 `## 标题` 切分区块，`grep`/`sed` 抽名字，`column` 排版。
- **踩坑**：
...
- **适用范围**：把 OS 课每卷精选命令存成卡片，终端里随查随用；已存 40 张（uname/sysctl/system_profiler/vm_stat/powermetrics/iostat/top·htop/ps/kill/launchctl/sw_vers/uptime/ls/mount/stat/ln/df/du/find/xattr/echo $PATH/export/alias/type/which/source/zsh -  n/cat ~/.zshrc/brew/npm/ln -sf，以及 git 实战 9 张：git status / git log / git add·commit / git branch·switch / git fetch·pull·push / git stash / git diff / git remote·clone / git 别名）。
- **关联 OS 课卷**：第二卷·文件系统（cheatsheet.md 这个文件本身）、第三卷·Shell 与终端（zsh 函数 / alias / 文件测试 / `command_not_found_handler`）、第四卷·环境变量分裂（GUI 不加载 .zshrc 的坑）。
...
#   pycheat --llm <说法>   （预留）LLM 语义增强，未配置则退回本地
...
def main():
    ...
    if args.llm: print("（LLM 模式预留：当前未配置模型/密钥，已退回本地匹配）")
    ranked = sorted(((score_card(c,q),c) for c in cards if score_card(c,q) > 0), key=lambda x:-x[0])
    if not ranked:
...
- **关键技巧**：
  ...
  - **`--llm` 预留**：代码里留了接口位，将来接模型时只需在 `main()` 的 `--llm` 分支把 query 发给模型拿到候选卡名，再走本地 `render`；默认关，不影响离线使用。
...
- **踩坑**：
  ...
  - 旧 `cheat`（zsh + fzf）**继续可用**，两者并存；等 `pycheat` 用顺了再决定要不要替换。
...
> **你不需要记住任何命令。** 你只需要两种能力：① 用 `cheat` 按「想做的事」找到它；② 用 `-c` 把示例命令复制下来直接跑。

```mermaid
flowchart TD
    A[我想做某件事] --> B{记得命令名?}
    B -->|记得| C["cheat df<br/>精确打印卡片"]
    B -->|不记得| D["cheat 内存 / cheat 磁盘<br/>按内容或中文搜"]
    D --> E[fzf 选一张卡]
    E --> F[看到示例命令]
    F --> G["cheat -c df<br/>示例直接进剪贴板, ⌘V 粘贴跑"]
    H[随手敲错命令] --> I["command_not_found 钩子<br/>自动提示 cheat 里相关的卡"]
```
...
| 你想干嘛 | 敲什么 | 说明 |
|----------|--------|------|
| 浏览/漫游全部命令 | `cheat` 回车 | fzf 模糊选（装了 fzf） |
| 记得名字，看详情 | `cheat df` | 精确打印卡片 |
| **不记得名字，只记得想干嘛** | `cheat 内存` / `cheat 磁盘` / `cheat 进程` | 按卡片内容/中文搜 |
| 只记得名字开头几个字母 | `cheat sys` / `cheat unam` | 按命令名子串搜 |
| **把示例命令复制下来直接跑** | `cheat -c df` | 进剪贴板，`⌘V` 即可运行 |
| 列出全部命令名 | `cheat -l` | 复习/漫游用 |
| 敲错命令被提醒 | （随便敲个错字） | zsh 自动提示 cheat 里相关的 |
| **git 想干嘛就搜啥** | `cheat 提交` / `cheat 分支` / `cheat 同步` / `cheat 暂存` | 按中文意图命中 git 卡 |
| git 精确卡 | `cheat git status` / `cheat git log` | 打印完整卡片 |
...
- **每次学到新命令，顺手补一张卡**：往 `~/.config/cheat/cheatsheet.md` 末尾加 `## 命令名` 区块（`cheat` 自动识别，不用重启）。卡片字段固定为：作用 / 用法 / 示例 / 踩坑 / Windows对照 / 来源卷。
- **给命令起「人话别名」**：如果某个命令你老忘，在 `~/.zshrc` 加 `alias` 或把逻辑抽成 `cheat` 里的一张卡。
- **用 `-c` 代替手敲**：凡是示例命令长的，直接 `cheat -c <名字>` 复制，避免敲错。
...
`cheat` 现在已是「交互浏览 + 回退/前进」的可用版本。下面按「信号价值」排序的几个下一步，挑你顺手的做：

| 优先级 | 方向 | 能解决什么 | 怎么落地（轻量） |
|--------|------|-----------|------------------|
| ★★★ | **收藏 / 高频置顶** | 你常查的命令总在翻，应该一打开就在前面 | 在 cheatsheet 加 `★` 标记，`_cheat_all_names` 时把带 `★` 的排到最前 |
| ★★★ | **左分类 / 侧边树** | 40+ 卡之后纯线性翻太累，按「卷 / 类别」分组跳转 | 卡片加 `分类:` 字段，`cheat` 启动先选分类再浏览 |
...
| ★☆☆ | **LLM 语义查询**（阶段2·离线版已落地） | `pycheat "怎么看哪个进程占内存"` 自然语言命中 | **已用 Python CLI（`pycheat`）+ 别名/中文二元组做到离线命中**；`--llm` 开关预留，将来接模型做 embedding 检索 |
...
```bash
# —— cheat 的安装（已存在，备查）——
# 源文件 ~/.cheat.zsh 里定义 cheat 函数 + alias c + command_not_found_handler
# ~/.zshrc 第 8 行：[ -s "$HOME/.cheat.zsh" ] && \. "$HOME/.cheat.zsh"
# 数据源：~/.config/cheat/cheatsheet.md
# 每学完一卷，往 cheatsheet.md 末尾加一个 `## 命令名` 区块即可，cheat 自动识别
```
...
2. 新机器拉取后：
   ...
   - 跑上面的软链命令（`ln -s`）
   - 把 `source ~/.cheat.zsh`、`export PATH="$HOME/bin:$PATH"` 加进新机器的 `~/.zshrc`
3. 这样「造一次，处处可用」。

## 附录 C：维护约定

- **工具卡**：每造一个工具 +1 张卡；字段严格按 §0.2。
- **命令卡**（`cheat` 数据源）：每学完 OS 课一卷，往 `cheatsheet.md` 补该卷精选命令。
- **升级路径**（cheat 专属）：阶段1 zsh 函数 + fzf（已做）→ 阶段2 转 Python CLI（接 LLM 语义查询）→ 阶段3 做 AI Skill（slash command）——**数据源 cheatsheet.md 不动**。
